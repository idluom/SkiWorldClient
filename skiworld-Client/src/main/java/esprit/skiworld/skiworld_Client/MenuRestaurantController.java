...
public class MenuRestaurantController implements Initializable{
	...
	public void products(ActionEvent event) {
		try {
			rootPane = (AnchorPane) FXMLLoader.load(getClass().getResource("/fxml/Product.fxml"));
			PaneDrawer.setContent(rootPane);
			PaneDrawer.setDefaultDrawerSize(0);
			PaneDrawer.toFront();
			hamburger.toFront();
	...
	@FXML
	public void dailyMenu(ActionEvent event) {
		try {
			rootPane = (AnchorPane) FXMLLoader.load(getClass().getResource("/fxml/.fxml"));
			PaneDrawer.setContent(rootPane);
			PaneDrawer.setDefaultDrawerSize(0);
			PaneDrawer.toFront();
			hamburger.toFront();
