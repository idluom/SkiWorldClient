	...
	iq.setText(ShopController.getE().getInventoryquantity().toString());
	  ...
	  @FXML
	    void back(ActionEvent event) {
		  try {
				MainApp.changeScene("/fxml/Shop.fxml", "UpdateShop");
			} catch (IOException e) {
				e.printStackTrace();
			}
	    ...
	    void modifier(ActionEvent event) {
	    	...
	    	if(name.getText().length()==0|| category.getText().length()==0 || price.getText().length()==0 || sq.getText().length()==0||iq.getText().length()==0)
